...
namespace MechanicShop.Application.UnitTests.Behaviours
{
    public class CachingBehaviourTests
    {
        ...
        private readonly ILogger<CachingBehaviour<CachedQuery, Result<string>>> _logger = Substitute.For<ILogger<CachingBehaviour<CachedQuery, Result<string>>>>();

        private readonly CachingBehaviour<CachedQuery, Result<string>> _sut;

        public CachingBehaviourTests()
        {
            _sut = new CachingBehaviour<CachedQuery, Result<string>>(_cache, _logger);
        }
        ...
        public async Task Handle_WhenNotCachedQuery_ShouldSkipCacheAndReturnResult()
        {
            ...
            var behavior = new CachingBehaviour<NonCachedQuery, string>(_cache, Substitute.For<ILogger<CachingBehaviour<NonCachedQuery, string>>>());
