...
using MechanicShop.Application.Features.RepairTasks.Commands.RemoveRepairtTask;
...
namespace MechanicShop.Api.Controllers
{
    ...
    {
        [HttpGet]
        [ProducesResponseType(typeof(List<RepairTaskDto>), statusCode: StatusCodes.Status200OK)]
        ...
        [ProducesResponseType(typeof(RepairTaskDto), statusCode: StatusCodes.Status200OK)]
        ...
        [MapToApiVersion("1.0")]
        public async Task<IActionResult>Create(CreateRepairTaskRequest request, CancellationToken ct)
        {
            var parts = request.Parts.ConvertAll(p => new CreateRepairTaskPartCommand(p.Name, p.Quantity, p.Cost));

            var result = await sender.Send(new CreateRepairTaskCommand(request.Name,
                request.LaborCost,
                (RepairDurationInMinutes)(int)request.EstimatedDurationInMins,
                parts),
                ct);
        ...
        {
            var parts = request.Parts.ConvertAll(p => new UpdateRepairTaskPartCommand(p.PartId, p.Name, p.Quantity, p.Cost));

            var result = await sender.Send(
                new UpdateRepairTaskCommand(repairTaskId,
                    request.Name,
                    request.LaborCost,
                    (RepairDurationInMinutes)(int)request.EstimatedDurationInMins,
                    parts),
        ...
        [MapToApiVersion("1.0")]
        public async Task<IActionResult> Delete(Guid repaidTaskId, CancellationToken ct)
        {
            var result = await sender.Send(new RemoveRepairTaskCommand(repaidTaskId), ct);
