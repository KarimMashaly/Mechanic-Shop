...
namespace MechanicShop.Application.Features.Dashboard.Queries.GetWorkOrderStates
{
    public class GetWorkOrderStatsQueryHandler(IAppDbContext context) : IRequestHandler<GetWorkOrderStatsQuery, Result<TodayWorkOrderStatsDto>>
    {
        ...
        public async Task<Result<TodayWorkOrderStatsDto>> Handle(GetWorkOrderStatsQuery query, CancellationToken ct)
        {
            var start = query.Date.ToDateTime(TimeOnly.MinValue, DateTimeKind.Utc);
            var end = query.Date.AddDays(1).ToDateTime(TimeOnly.MinValue, DateTimeKind.Utc);
            ...
            var stats = await workOrders.ToListAsync(ct);

            var totalRevenue = stats.Sum(s => s.Invoice?.Total ?? 0);
