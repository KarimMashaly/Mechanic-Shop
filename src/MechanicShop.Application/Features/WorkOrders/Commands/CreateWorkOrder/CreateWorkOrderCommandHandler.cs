...
namespace MechanicShop.Application.Features.WorkOrders.Commands.CreateWorkOrder
{
        ...
        : IRequestHandler<CreateWorkOrderCommand, Result<WorkOrderDto>>
    {
        public async Task<Result<WorkOrderDto>> Handle(CreateWorkOrderCommand command, CancellationToken ct)
        {
            ...
            var totalEstimatedDuration = TimeSpan.FromMinutes(repairTasks.Sum(t => (int)t.EstimatedDurationInMins));
            var endAt = command.StartAtUtc.Add(totalEstimatedDuration);

            if(workOrderPolicy.IsOutsideOperatingHours(command.StartAtUtc, totalEstimatedDuration))
            {
                logger.LogError("The WorkOrder time ({StartAt} ? {EndAt}) is outside of store operating hours.", command.StartAtUtc, endAt);

                return ApplicationErrors.WorkOrderOutsideOperatingHour(command.StartAtUtc, endAt);
            }

            var checkMinRequirementResult = workOrderPolicy.ValidateMinimumRequirement(command.StartAtUtc, endAt);
            ...
            var checkSpotAvailabilityResult = await workOrderPolicy.CheckSpotAvailabilityAsync(
                    command.Spot,
                    command.StartAtUtc, 
                    endAt,
            ...
            var vehicle = await context.Vehicles
                                    ...
                                    .FirstOrDefaultAsync(v => v.Id == command.VehicleId);
            ...
            var hasVehicleConflict = await context.WorkOrders
                                          . AnyAsync(a =>
                                            ...
                                            a.StartAtUtc.Date == command.StartAtUtc.Date &&
                                            a.StartAtUtc < endAt &&
                                            a.EndAtUtc > command.StartAtUtc,
                                            ct);
