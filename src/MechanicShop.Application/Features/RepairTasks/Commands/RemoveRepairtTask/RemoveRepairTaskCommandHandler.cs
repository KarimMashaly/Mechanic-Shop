...
namespace MechanicShop.Application.Features.RepairTasks.Commands.RemoveRepairtTask
{
        ...
        : IRequestHandler<RemoveRepairTaskCommand, Result<Deleted>>
    {
        ...
        {
            var repairTask = await context.RepairTasks.FindAsync([command.RepairTaskId], ct);
            ...
            if(isInUse)
            {
                logger.LogWarning("RepairTask {RepairTaskId} cannot be deleted — in use by work orders.", command.RepairTaskId);
